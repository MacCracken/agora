...
**M5-G landed 0.3.0+**: per-store flock (`<store>/.lock`) around the `post_max_id` + EXCL-claim + write critical section. `store_lock_acquire` / `store_lock_release` helpers via `lib/io.cyr` `file_lock`. Pre-M5-G a race would drop one of two concurrent posts (EEXIST); post-M5-G both serialize and succeed with distinct IDs. 50-parallel-CLI smoke confirms.
