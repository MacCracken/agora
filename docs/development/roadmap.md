...
**1.3.7 War-game — next.** An **asynchronous shared-world strategy** door ([ADR 0014](../adr/0014-async-shared-world-strategy.md)): a common map, alliances, turn-batched combat resolution between callers. The new-capability proof — it deliberately exercises the shared-state-mutation-between-callers path on the 1.2.0 `flock`'d world-transaction framework ([ADR 0010](../adr/0010-persistent-universe.md)), with turn resolution lazy on caller entry — retiring real concurrency risk as the on-ramp to the **1.4.0 Descent link** (BBS → MUD gateway). After 1.4.0, the Olympiad's later events (gladiators, athletics, boat crews) and deeper-Universe work stay unpinned. See the release table + *Planned* below.
