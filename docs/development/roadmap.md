...
**1.3.4 Wager — next.** A single shared casino/wagering module (`src/wager.cyr`, [ADR 0013](../adr/0013-wagering-module-rng-fairness.md)) — the build-once primitive (bet validation, payout tables, the kernel-CSPRNG entropy draw, house edge) every game will call, distinct from the games' replayable seeded PRNG. After it: **1.3.5–1.3.7** — **casino integrations** across the existing doors → a **training sim** (QUEST-economy reskin + the wager flagship) → a **war-game** (async shared-world strategy, the deliberate MUD on-ramp) — then **1.4.0 Descent link**. See the release table + *Planned* below.
