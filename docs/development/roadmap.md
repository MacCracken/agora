...
| **1.3.1** | **PARRY** (Colby, 1972) — the paranoid foil to Eliza on the same reusable chatbot core: `play parry` + `/parry`. Reuses the 1.3.0 primitives as-is (`ez_normalize`/`ez_reflect`/`ez_scan`/memory ring + door/side-channel shells); **first step** is factoring `ez_respond`/`ez_resp_none` into a script-descriptor (keyword table + response-fn table + opener/goodbye/deflection strings), after which PARRY is purely a new descriptor. | 📋 planned |
