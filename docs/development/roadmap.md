...
**1.3.5 Casino integrations — next.** Embed the shipped 1.3.4 wager module (`src/wager.cyr`) across the **existing** doors so it earns its keep: a cantina gambling table in **Port Authority**, a back-alley dice game in **Smuggler's Ledger**, a tavern card game in **QUEST**. Same module, three payout tables, three contexts — no fifth thing to maintain, and the wager surface becomes user-reachable for the first time (it ships NOPed at 1.3.4). After it: **1.3.6** training sim (QUEST-economy reskin + the wager flagship) → **1.3.7** war-game (async shared-world strategy, the deliberate MUD on-ramp) → **1.4.0 Descent link**. See the release table + *Planned* below.
