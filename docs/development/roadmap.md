...
**1.3.6 Training sim — next.** A fighter/horse **training + competition** door: the cheap reskin of QUEST's daily-turn economy (rationed daily training → stats improve → periodic competition resolves) in a sports skin, and the **wager module's flagship integration** (bet on your own fighter / on the races). Two of the three casino-arc decisions reinforcing each other on the shipped 1.3.5 wager surface. After it: **1.3.7** war-game (async shared-world strategy, the deliberate MUD on-ramp) → **1.4.0 Descent link**. See the release table + *Planned* below.
