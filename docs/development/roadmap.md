...
| **1.3.3** | **Jabberwacky** (Carpenter, 1988→) — a **corpus-learning** chatbot: a *different engine* from the ELIZA/PARRY fixed-script core (retrieval over a growing conversation corpus), so it brings agora's first learning / persistent-state bot. `play jabberwacky` + `/jabberwacky`. | 📋 planned |
| **1.3.4** | **Wager** ([ADR 0013](../adr/0013-wagering-module-rng-fairness.md)) — one shared casino/wagering module (`src/wager.cyr`), the **build-once primitive**: bet validation, payout tables, the entropy draw, house edge. A *mechanic, not a door* — the same loop (bet → draw → resolve → settle) every game calls; the "one shared abstraction under many games" pattern (cf. the door PRNG). Decision: the draw pulls from the **kernel CSPRNG** (non-replayable), distinct from the games' replayable seeded `door.cyr` PRNG. | 📋 planned |
| **1.3.5** | **Casino integrations** — embed the 1.3.4 wager module across the **existing** doors so it earns its keep: a cantina gambling table in **Port Authority**, a back-alley dice game in **Smuggler's Ledger**, a tavern card game in **QUEST**. Same module, many contexts — every game gets richer with no fifth thing to maintain. | 📋 planned |
| **1.3.6** | **Training sim** — a fighter/horse **training + competition** door: the cheap reskin of QUEST's **daily-turn economy** (rationed daily training → stats improve → periodic competition resolves) in a sports skin, and the **wager module's flagship integration** (bet on your own fighter / on the races). Two decisions reinforcing each other. | 📋 planned |
| **1.3.7** | **War-game** ([ADR 0014](../adr/0014-async-shared-world-strategy.md)) — an **asynchronous shared-world strategy** door (a common map, alliances, turn-batched combat resolution between callers). The high-value **new-capability proof**: it deliberately exercises the shared-state-mutation-between-callers path on the 1.2.0 `flock`'d world-transaction framework ([ADR 0010](../adr/0010-persistent-universe.md)), with turn resolution **lazy on caller entry** (a resolution daemon kept open as a deferred future feature), retiring real concurrency risk as the **on-ramp to the 1.4.0 Descent/MUD** real-time world. | 📋 planned |
...
**1.3.3 Jabberwacky — next.** Rollo Carpenter's 1988 corpus-learning chatbot — a *different engine* from the fixed-script Eliza/PARRY core (retrieval over a growing conversation corpus + persistent state), so it brings agora's first learning / persistent-state bot: `play jabberwacky` + `/jabberwacky`. After it: **1.3.4–1.3.7** — the shared **wager** module (built once) → **casino integrations** across the existing doors → a **training sim** (QUEST-economy reskin + the wager flagship) → a **war-game** (async shared-world strategy, the deliberate MUD on-ramp) — then **1.4.0 Descent link**. See the release table + *Planned* below.
...
**Next planned (see *Planned* below):** 1.3.1 PARRY (reuses the Eliza core; first step is the script-descriptor refactor); 1.4.0 Descent link (BBS → MUD gateway). Deeper Universe (PA alliances/mines, Handler intercepts/sabotage, Smuggler aggregate price pressure), further chatbot personalities, and the **QUEST** LORD-homage door ([`roadmap-future.md`](roadmap-future.md) § Door games / § Chatbot personalities) stay unpinned until pulled.
