...
**Alchemical spine (the buried reference — comedic surface, hermetic skeleton, neither breaks character to nod at the other).** The twelve-master curve maps cleanly onto the **Great Work** (*magnum opus*): four stages — **nigredo** (blackening), **albedo** (whitening), **citrinitas** (yellowing), **rubedo** (reddening) — three masters per stage, giving each forest tier a colour-and-mood arc that reads as mythic *without ever explaining itself* (the masters are tempered by their stage; nobody lectures alchemy). The title decodes the goal: "**Undying … Sovereign Throne**" is the *philosopher's-stone* prize — immortality + the perfected king (the alchemical *rex*) — so the Slay-the-Dragon endgame becomes **attaining the Throne = completing the Work**, and the dragon is the **prima materia** you confront and *transmute*, not merely a boss you kill. The **Emerald** of the title is the through-line item: a tablet the player reassembles across the twelve levels, **each master holding one fragment** — a clean mechanical spine (shard-gated progression, exactly how LORD already gates advancement on masters) that is secretly the recovery of the **Tabula Smaragdina** (the Emerald Tablet of Hermes Trismegistus — *as above, so below*). The player thinks they're collecting MacGuffin shards; the design knows they're recovering the Emerald Tablet. The two layers *are* the game: LORD's comedic, self-aware, slightly-bawdy voice (the load-bearing tone above) on top, the Work underneath.

**Fit summary:** pure-module state machine (ADR 0009) ✓ already the house pattern; daily-turn cadence ✓ already in the games; shared character store + async PvP ✓ the 1.2.0 world transaction + PA garrison model; leaderboards ✓ the `scores` surface. The genuinely new work is the **town/flavor content + the comedic voice** — i.e. writing, which is the point. Unpinned; pull when the user asks.
...
- **Jabberwacky** (Rollo Carpenter, 1988→) — **learning by retrieval** from a growing corpus of past conversations. Like MegaHAL, a corpus-learning engine rather than a fixed script; the most ambitious (and the one with the heaviest persistent-state + moderation story).
