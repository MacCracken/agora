...
> **Last refresh**: 2026-06-08 (**1.2.0 Persistent Universe CUT** — shared-world multiplayer for all three door games [ADR 0010, all 6 bites]: world-transaction framework, PA shared galaxy [depletable stock + planet ownership] + async-PvP garrisons, Smuggler shared heat, Handler shared city alerts, cross-game leaderboards [`scores <game>`]. **141/141**, 678,776 B; smokes 08/09/10 green. The array-codegen blocker is cleared on 6.1.5 [t129]. Roadmap: **Eliza + chat area** = planned 1.3.0; **MUD link via cyrius-yeomans-descent** = planned 1.4.0.) Prior: **1.1.1** Handler field pressure + toolchain unblock 6.0.52 → **6.1.5**; **1.1.0** door / games; **1.0.0** cut 2026-05-23 (iron-validated on archaemenid). | **Refresh cadence**: every release; ideally bumped by the release post-hook.
...
| Field | Value |
|---|---|
| **Released** | `1.2.0` (2026-06-08) — **Persistent Universe** (ADR 0010): shared-world multiplayer for all three door games — world-transaction framework, PA shared galaxy + async-PvP garrisons, Smuggler shared heat, Handler shared city alerts, cross-game leaderboards. Built on `1.1.1` (Handler field pressure + 6.1.5 toolchain unblock), `1.1.0` (door / games, ADR 0009), `1.0.0` (2026-05-23, iron-validated on archaemenid). |
...
|---|---|---|
| `build/agora` (DCE) | **678,776 B** at 1.2.0 on cyrius 6.1.5 (clean `rm -rf build && cyrius deps && CYRIUS_DCE=1 cyrius build`) — was 662,608 B at 1.2.0-bite-2, 654,592 B at 1.1.1; +24,184 B across the full Universe (bites 2-5: PA world+PvP, Smuggler heat, Handler alerts, leaderboards). The 484,184 B at 1.1.0 was on 6.0.52 (bulk of that gap is the toolchain codegen change). DCE NOPs unreachable fns in place rather than stripping. | `CYRIUS_DCE=1 cyrius build src/main.cyr build/agora` |
| `build/test` | 141 tests | `cyrius test src/test.cyr` |
...
**1.2.0 — Persistent Universe — CUT 2026-06-08. No active cycle.** All six ADR 0010 bites shipped; shared-world multiplayer for all three door games. Summary:
...
- **1.3.0 — Chat area + Eliza** (planned, see roadmap): a live multi-user chat surface with a pure-module Rogerian chatbot; builds on the 1.2.0 shared-disk framework.
- **1.4.0 — MUD link via `cyrius-yeomans-descent`** (planned, see roadmap): wire the sibling MUD userland in as a linked door/destination over the shared telnet substrate.
- **Deeper Universe** (unpinned): PA alliances + mines + the full upgrade tree; Handler intercept pool + anonymous-tip sabotage; Smuggler aggregate price pressure (beyond heat). These are the ADR 0010 §-per-game ideas not yet built.
...
- `src/main.cyr` — `MODE_DOOR` + `play <game> [practice|solo|universe]` + door glue. **1.2.0**: generic universe world-transaction (`door_world_begin`/`door_world_commit`/`door_universe_feed`/`door_universe_save`/`door_set_world`, dispatch by game) + `g_uni_world` buffer; **cross-game leaderboards** (`door_leaderboard_path`/`door_post_score`/`door_render_leaderboard`) + `scores <game>` command
