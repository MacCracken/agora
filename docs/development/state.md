...
| Field | Value |
|---|---|
| **Released** | `1.3.1` (2026-06-08) — **PARRY** (Colby 1972): the paranoid foil to Eliza as an **affect engine** (fear/anger/mistrust → mood-gated dispatch + a Mafia/bookie delusion story), `play parry` + a private `/parry` side-channel; reuses the ELIZA text primitives. Built on `1.3.0` (Chat area + Eliza, ADR 0011), `1.2.0` (Persistent Universe, ADR 0010), `1.1.x` (door / games), `1.0.0` (2026-05-23, iron-validated on archaemenid). |
...
|---|---|---|
| `build/agora` (DCE) | **752,240 B** at 1.3.1 on cyrius 6.1.9 (clean `rm -rf build && cyrius deps && CYRIUS_DCE=1 cyrius build`) — was 730,792 B at 1.3.0; +21,448 B for `src/parry.cyr` + the GAME_PARRY/`/parry` wiring. (1.2.0 was 678,776 B on 6.1.5; the 1.2.0→1.3.0 jump also folded the 6.1.5→6.1.9 toolchain codegen delta.) DCE NOPs unreachable fns in place rather than stripping. | `CYRIUS_DCE=1 cyrius build src/main.cyr build/agora` |
| `build/test` | 160 tests | `cyrius test src/test.cyr` |
...
|---|---|---|---|
| cyrius | self | 6.1.9 (pinned; realigned 6.1.5 → 6.1.9 at the 1.3.0 cut; earlier lifted from the 6.0.52 sigil-SIGILL cap on 2026-06-08) | ✅ |
...
- `src/main.cyr` — `MODE_DOOR` + `MODE_CHAT` + `play <game> [practice|solo|universe]` (now incl. `eliza`, forced practice) + door glue. **1.2.0**: generic universe world-transaction + `scores <game>`. **1.3.0**: the `chat [channel]` command, `chat_cmd_dispatch` (`/leave`/`/help`/`/eliza`), `chat_flush_tail`, the `MODE_CHAT` line dispatch + recv-timeout live-tail tick (`EWOULDBLOCK_ERR`/`CHAT_POLL_SECS`/`CHAT_IDLE_SECS`), and the chatbot side-channel — **1.3.1** generalized to `g_chat_bot` (0/1/2) + `g_eliza_state`/`g_parry_state` + `chat_bot_state`/`chat_bot_feed`/`chat_bot_name`, `/eliza` + `/parry`; GAME_PARRY door wiring
- `src/test.cyr` — **160-test** suite (+5 PARRY at 1.3.1: t156-t160 — classifier flags, affect dynamics [flare spike + benevolent calm with sticky mistrust], mood thresholds, story latch + beat advance, steer-back; over the +14 chat/Eliza at 1.3.0: t142-t148 chat transcript helpers, t149-t155 Eliza engine incl. the t155 reflection-grammar guard)
