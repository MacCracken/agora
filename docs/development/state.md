...
> **Last refresh**: 2026-06-08 (**1.2.0 Persistent Universe — bite 2 landed**: Port Authority *shared galaxy* [depletable port stock that moves the next player's price + exclusive planet ownership by sigil fp], `play port universe`, the per-line `flock`'d world transaction; the canonical Universe slice, the previously-reverted bite re-authored on 6.1.5. **135/135** [t128-t135, incl. t129 the codegen-bug probe], 662,608 B; cross-session shared-world smoke `09-universe-port.sh` green. VERSION stays **1.1.1** — 1.2.0 not cut [bites 3-6 remain]. Also: **Eliza + chat area** penciled as planned 1.3.0 in roadmap.) Prior: **1.1.1 released** — The Handler field pressure (127/127) + toolchain unblock 6.0.52 → **6.1.5** (sigil SIGILL cleared). Prior: **1.1.0** door / games; **1.0.0** cut 2026-05-23 (iron-validated on archaemenid). | **Refresh cadence**: every release; ideally bumped by the release post-hook.
...
|---|---|---|
| `build/agora` (DCE) | **662,608 B** on cyrius 6.1.5 with 1.2.0 bite 2 (clean `rm -rf build && cyrius deps && CYRIUS_DCE=1 cyrius build`) — was 654,592 B at 1.1.1 (+8,016 B for the PA shared-world code); 484,184 B at 1.1.0 on 6.0.52 (the bulk of the gap to there is the 6.0.52 → 6.1.5 toolchain codegen change). DCE NOPs unreachable fns in place rather than stripping. | `CYRIUS_DCE=1 cyrius build src/main.cyr build/agora` |
| `build/test` | 135 tests | `cyrius test src/test.cyr` |
...
**1.2.0 — Persistent Universe (active cycle — bites 1-2 done, bite 3 next).** Shared-world multiplayer for the three door games. **Everything before 1.2.0 is shipped history** — the 0.x line → the 1.0.0 BBS cut → the **1.1.0** door / games subsystem → **1.1.1** Handler field pressure (all single-player). Design for 1.2.0 is locked in [ADR 0010](../adr/0010-persistent-universe.md): a per-game world dir under `<store>/.games/<game>/world/`, mutated via a `flock`'d lock→read→compute→write transaction with the game logic staying a **pure transform** (the ADR 0009 pure-module rule survives); Universe requires login; async/indirect PvP, not real-time. Bite plan + scope in [`roadmap.md`](roadmap.md) § In progress.
...
**Bite 2 (Port Authority shared galaxy) shipped 2026-06-08** — the canonical Universe slice; the previously-reverted bite, re-authored and landed on cyrius 6.1.5. What's in it:
- **World model** (`port_authority.cyr` § Universe): a flat-i64 snapshot — version + seed + per-(sector,commodity) **stock** + per-sector **planet-owner fp** (`paw_byte_size()` = 2016 B for the 50-sector × 4-commodity galaxy), persisted verbatim by the bite-1 `world_read`/`world_write`. Pure transforms `paw_buy`/`paw_sell` (stock moves the next quote via `paw_price`, clamped `[base/3, base*2]`) + `paw_claim_planet` (exclusive by fp). Ship-side glue `pa_is_universe`/`pa_buy_u`/`pa_sell_u` routes the *existing* PA screen machine through the world; `pa_establish_planet` claims in-world.
- **main.cyr wiring**: `play port universe` (login-gated) loads/creates the player's `portu` ship save + primes the shared world; `door_feed_line` → `pa_universe_feed` wraps every input line in `pa_world_begin` (`flock` + read/create snapshot) → `pa_feed` → `pa_world_commit` (write + unlock). The galaxy regenerates from a fixed `PA_UNIVERSE_SEED` (= 2002) so all players share one map.
- **Tests** t128-t135 (135/135). **t129 is the codegen-bug probe** — a distinct-write-readback over the whole stock array (`store64(base+i*8,…)` + accessor reads in sibling nested loops); it passes, so the array-in-loop bug that reverted the first attempt is **cleared on 6.1.5 with the real pattern**, not just a synthetic probe. End-to-end `docs/examples/09-universe-port.sh`: two players share one galaxy, planet ownership is exclusive across sessions, world snapshot persists, anonymous universe play is refused.

**Bite 3+ remaining** (per ADR 0010 § Phasing): 3 = PA deployments + async PvP (fighters/mines vs left-behind assets, alliances); 4 = Smuggler shared economy + Handler shared alerts/intercepts/sabotage; 5 = leaderboards generalized from the Handler standings; 6 = closeout + VERSION → 1.2.0. **VERSION stays `1.1.1`** until the full 1.2.0 cut. The original two blockers are both retired: the sigil/crypto SIGILL (cleared on 6.1.x) and the array-in-loop codegen bug (re-cleared on the real bite-2 code via t129). The slot description below is prior post-1.0 context, retained.
...
- `src/port_authority.cyr` — Port Authority: galaxy gen + port economy + nav/turns/combat + HQ dock + planet + feed/render + save (header `PADG1`). **1.2.0 bite 2 Universe** (ADR 0010): `paw_*` shared-world model (flat-i64 snapshot: stock + planet-owner fp) + pure transforms (`paw_buy`/`paw_sell`/`paw_price`/`paw_claim_planet`) + ship-side glue (`pa_is_universe`/`pa_buy_u`/`pa_sell_u`/`pa_shown_price`); `PA_WORLD`/`PA_FP` runtime fields; `pa_establish_planet` claims in-world
...
- `src/main.cyr` — `MODE_DOOR` + `play <game> [practice|solo]` verb + door glue (`door_send_frame`/`door_feed_line`/`door_save_on_exit`/`door_append_standing`). **1.2.0 bite 2**: `play port universe` (login-gated) + the world-transaction glue (`pa_world_begin`/`pa_world_commit`/`pa_universe_feed`/`pa_universe_save`) + `g_pa_world` session buffer
