...
| Field | Value |
|---|---|
| **Released** | `1.3.0` (2026-06-08) — **Chat area + Eliza** (ADR 0011): the synchronous public-assembly surface — a live multi-user `flock`'d chat area (`MODE_CHAT`, seq-number live-tail) + **Eliza** (Weizenbaum 1966 DOCTOR) as a `play eliza` door and a private `/eliza` chat side-channel. Built on `1.2.0` (Persistent Universe, ADR 0010), `1.1.x` (door / games), `1.0.0` (2026-05-23, iron-validated on archaemenid). |
...
|---|---|---|
| `build/agora` (DCE) | **730,792 B** at 1.3.0 on cyrius 6.1.9 (clean `rm -rf build && cyrius deps && CYRIUS_DCE=1 cyrius build`) — was 678,776 B at 1.2.0 (on 6.1.5); +52,016 B for `src/chat.cyr` + `src/eliza.cyr` + the chat/Eliza wiring, plus the 6.1.5 → 6.1.9 toolchain codegen delta (the two are not separated). DCE NOPs unreachable fns in place rather than stripping (1006 NOPed at 1.3.0). | `CYRIUS_DCE=1 cyrius build src/main.cyr build/agora` |
| `build/test` | 155 tests | `cyrius test src/test.cyr` |
...
**1.3.0 — Chat area + Eliza — CUT 2026-06-08. No active cycle.** All three ADR 0011 bites shipped. Summary:
