...
- **Storage** — `<store>/.users/<fp16>/games/<game>.sav`, under the dot-prefixed `.users/` tree so it dodges board/post enumeration like account files. `game_name_valid` (board-name alphabet) is the *only* traversal guard on the filename; save read/write re-validate `fp16_valid` + `game_name_valid` at the boundary. Single-owner saves take no flock and, **since 1.7.0 (roadmap N2), are written with `store_write_atomic`** — temp + `fsync` + `rename` — rather than the plain `O_TRUNC` write they used through 1.6.7, which emptied the save at open and so lost a player's progress on any crash or short write. Shared world/standings state uses `file_append_locked`.
