...
- **Negative** — no single-sign-on yet: a citizen logs in twice (once to agora, once to the MUD). The proxy holds a forked worker for the whole MUD session (acceptable under fork-per-accept; bounded by the idle timeout).

> **Amendment (1.6.0 / 1.7.0).** This cost model assumed fork-per-accept was the only serve model. It is not > ([ADR 0023](0023-dual-serve-model.md)). Under `AGORA_SERVE=poll` — and on agnos, which has no choice — the > proxy blocks the shared sweep, so one player in the MUD stalls the other 63 sessions, bounded only by > `DESCENT_IDLE_MS` (30 minutes). **Since 1.7.0 that also delays clean shutdown**, because the poll loop's > signalfd check lives in the same sweep. Making Descent a *state* in the poll loop rather than a blocking > call is the standing fix; it is roadmap-tracked. `poll` is invoked via the same raw `syscall(7, …)` convention `net_connect_nb` already uses — an x86_64-Linux assumption that rides along with the existing listener (cross-arch parity is a `lib/net.cyr` task, per [ADR 0001](0001-cross-platform-listener-decoupled-from-agnos.md)).
