...
- **World transaction (the core pattern).** Any action that touches shared state runs:
  ...
  4. write the world back under the held lock. **1.2.0-1.6.7 did an in-place `O_TRUNC` write**, correct against cooperating lock-holders — proven by `08-world-concurrency.sh` — but not against a crash *during* the write, which empties the shared world for every player at once. **Since 1.7.0 (roadmap N2) it is a temp + `fsync` + `rename`** (`store_write_atomic`), i.e. the upgrade this step used to defer. ⚠ **The lock/data separation in step 1 is now LOAD-BEARING, not incidental**: `rename` replaces the inode and `flock` binds to an inode, so if the lock were ever moved onto `snapshot` itself this ADR's no-lost-updates guarantee would evaporate silently — every later writer would lock the *new* inode while an in-flight reader still held the old one. The lock stays on `<world>/.lock`. The 8×300-transaction smoke still lands exactly 2400;
  5. release the lock.
