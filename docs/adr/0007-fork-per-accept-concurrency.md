...
- **(q) `SIG_IGN` / (r) `SA_NOCLDWAIT`** auto-reap: rejected because cyrius's `lib/darshana.cyr` already documents the rt_sigaction trampoline trap on x86_64. Using `sys_waitpid(-1, NULL, WNOHANG)` from the accept-loop avoids the sigaction surface for *reaping* and stays cross-arch-clean.
  > **Amendment (1.7.0).** The claim that this "avoids the sigaction surface entirely" stopped being   > true at **1.6.2**, when `signal_ignore(SIGPIPE)` — which builds an `rt_sigaction` act itself —   > became mandatory under the poll model. The trampoline objection was the real content and it still   > holds: cyrius 6.4.51's `signal_ignore` needs no `sa_restorer`, and **N1's clean shutdown installs   > no handler at all**, routing signals to a signalfd instead. So (q)/(r) remain rejected, but for   > the narrower reason — auto-reap hides exit status and we want the explicit drain — not because   > agora refuses to touch signals.

## Specifics

- **Fork point**: in `cmd_serve_on` (`src/main.cyr`), after `sock_accept` succeeds, before the child runs `handle_client`.
- **Child responsibilities**: **restore the default signal disposition first** (1.7.0 — `shutdown_signals_child_reset`: `fork(2)` inherits the parent's blocked mask, so once N1 armed SIGINT/SIGTERM in the parent every child was **unkillable** until it unblocked and closed the inherited signalfd); then close the parent's listening fd (`sock_close(sfd)`) so a child crash doesn't leak fds to grandchildren; run `handle_client(cfd)`; `sys_exit(0)` regardless of return value (the parent doesn't care).
- **Parent responsibilities**: close the accepted cfd (`sock_close(cfd)` — child has its own dup via fork); continue the accept loop. **Since 1.7.0** the parent also waits in `poll(2)` over {listener, signalfd} on a non-blocking listener rather than blocking in `accept`, and on shutdown closes the listener then reaps children for a bounded grace window — see [ADR 0023](0023-dual-serve-model.md) § Clean shutdown.
