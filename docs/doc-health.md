...
| 🟣 **Proposed (forward-looking)** | 2 | **ADR 0013** (shared wager module + RNG fairness, planned 1.3.4) and **ADR 0014** (war-game async shared-world strategy, the MUD on-ramp, planned 1.3.7) — decisions recorded ahead of the build, like ADR 0010 was at 1.2.0-open. They move to Accepted when their work cuts. |
...
| `CHANGELOG.md` | 2026-06-08 | ✅ Fresh | **Source of truth per CLAUDE.md.** [0.1.0] → **[1.3.3]** entered. [1.3.3] = Jabberwacky (corpus-learning bot, ADR 0015) + pin 6.1.10→6.1.12 + review fixes; [1.3.2] = QUEST (LORD-homage RPG) + pin 6.1.9→6.1.10 + 3 review fixes; [1.3.1] PARRY; [1.3.0] Chat + Eliza (ADR 0011); [1.2.0] Persistent Universe (ADR 0010); [1.1.x] door games; [1.0.0] release-narrative. |
...
| `VERSION` | 2026-06-08 | ✅ Fresh | `1.3.3`. Bumped via release flow. |
| `cyrius.cyml` | 2026-06-08 | ✅ Fresh | Toolchain pin **`6.1.12`** (realigned 6.1.10 → 6.1.12 at the 1.3.3 cut, 6.1.9 → 6.1.10 at 1.3.2, 6.1.5 → 6.1.9 at 1.3.0; earlier lifted from the 6.0.52 sigil-SIGILL cap; see state.md Version table). Deps list = 20 stdlib modules (no new deps through 1.3.x — chat/Eliza/PARRY/QUEST/Jabberwacky are all pure). |
...
| File | Last touched | Status | Action |
|---|---|---|---|
| `state.md` | 2026-06-08 | ✅ Fresh | **Rotates every release.** **1.3.3 CUT** — Jabberwacky (178 tests; 797,640 B; pin 6.1.12). Refresh line / Released / Toolchain / Build-artifacts / Tests / In-flight (Jabberwacky summary + 1.3.4 Wager next) / Recent-shipped rows synced. |
| `roadmap.md` | 2026-06-08 | ✅ Fresh | Release table: **1.3.3 ✅ Jabberwacky** ([ADR 0015](adr/0015-jabberwacky-corpus-learning.md)); planned **1.3.4 Wager ([ADR 0013](adr/0013-wagering-module-rng-fairness.md)) → 1.3.5 casino integrations → 1.3.6 training sim → 1.3.7 war-game ([ADR 0014](adr/0014-async-shared-world-strategy.md)) → 1.4.0 Descent link**. "In progress" → 1.3.4 next; the *Planned* § carries the 1.3.4–1.3.7 design blurb. |
...
15 ADRs (0001-0012 + 0015 Accepted; 0013/0014 Proposed for the 1.3.4 / 1.3.7 work). Re-read pass per minor closeout; ADRs document decisions, not status.
...
| `0013-wagering-module-rng-fairness.md` | 2026-06-08 | 🟣 Proposed (1.3.4) | **New** — the shared wager module + RNG fairness. A casino is a **mechanic, not a door** (one `src/wager.cyr`, many integrations). Core decision: the **draw** pulls from the **kernel CSPRNG** (`sys_getrandom` / `/dev/urandom`), distinct from the games' deliberately-seeded replayable `door.cyr` PRNG — fairness needs non-replayability, the opposite of the games' reproducibility. Pure validation/payout/edge/settlement (no negative-balance path), the draw the one injected impure seam. Rejects: standalone casino door, seeded draw, per-game bespoke, commit-reveal (first cut). |
