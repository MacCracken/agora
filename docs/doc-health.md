...
| `CHANGELOG.md` | 2026-06-08 | ✅ Fresh | **Source of truth per CLAUDE.md.** [0.1.0] → **[1.3.2]** entered. [1.3.2] = QUEST (LORD-homage RPG) + pin 6.1.9→6.1.10 + 3 review fixes; [1.3.1] PARRY; [1.3.0] Chat + Eliza (ADR 0011); [1.2.0] Persistent Universe (ADR 0010); [1.1.x] door games; [1.0.0] release-narrative. |
...
| `VERSION` | 2026-06-08 | ✅ Fresh | `1.3.2`. Bumped via release flow. |
| `cyrius.cyml` | 2026-06-08 | ✅ Fresh | Toolchain pin **`6.1.10`** (realigned 6.1.9 → 6.1.10 at the 1.3.2 cut, 6.1.5 → 6.1.9 at 1.3.0; earlier lifted from the 6.0.52 sigil-SIGILL cap; see state.md Version table). Deps list = 20 stdlib modules (no new deps through 1.3.x — chat/Eliza/PARRY/QUEST are all pure). |
...
| File | Last touched | Status | Action |
|---|---|---|---|
| `state.md` | 2026-06-08 | ✅ Fresh | **Rotates every release.** **1.3.2 CUT** — QUEST (166 tests; 782,064 B; pin 6.1.10). Refresh line / Released / Toolchain / Build-artifacts / Tests / In-flight (QUEST summary + 1.3.3 Jabberwacky next) / Recent-shipped / Source-surface (+quest.cyr) / Gate-state rows synced. |
| `roadmap.md` | 2026-06-08 | ✅ Fresh | Release table: **1.3.2 ✅ QUEST**; planned **1.3.3 Jabberwacky → 1.3.4 Wager ([ADR 0013](adr/0013-wagering-module-rng-fairness.md)) → 1.3.5 casino integrations → 1.3.6 training sim → 1.3.7 war-game ([ADR 0014](adr/0014-async-shared-world-strategy.md)) → 1.4.0 Descent link**. "In progress" → 1.3.3 next; the *Planned* § carries the 1.3.4–1.3.7 design blurb. |
...
14 ADRs (0001-0012 Accepted; 0013/0014 Proposed for the 1.3.4 / 1.3.7 work). Re-read pass per minor closeout; ADRs document decisions, not status.
