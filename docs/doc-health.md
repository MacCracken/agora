...
| `VERSION` | 2026-06-08 | ✅ Fresh | `1.3.4`. Bumped via release flow. |
| `cyrius.cyml` | 2026-06-08 | ✅ Fresh | Toolchain pin **`6.1.14`** (realigned 6.1.12 → 6.1.14 at the 1.3.4 cut, 6.1.10 → 6.1.12 at 1.3.3, 6.1.9 → 6.1.10 at 1.3.2, 6.1.5 → 6.1.9 at 1.3.0; earlier lifted from the 6.0.52 sigil-SIGILL cap; see state.md Version table). Deps list = 20 stdlib modules (no new deps through 1.3.x — chat/Eliza/PARRY/QUEST/Jabberwacky/Wager are all pure; Wager's CSPRNG draw uses the already-listed `syscalls`). |
...
| File | Last touched | Status | Action |
|---|---|---|---|
| `state.md` | 2026-06-08 | ✅ Fresh | **Rotates every release.** **1.3.4 CUT** — Wager (184 tests; 801,288 B; pin 6.1.14). Refresh line / Released / Toolchain / Build-artifacts / Tests / In-flight (Wager summary + 1.3.5 Casino integrations next) / Recent-shipped rows synced. |
| `roadmap.md` | 2026-06-08 | ✅ Fresh | Release table: **1.3.4 ✅ Wager** ([ADR 0013](adr/0013-wagering-module-rng-fairness.md), Accepted); planned **1.3.5 casino integrations → 1.3.6 training sim → 1.3.7 war-game ([ADR 0014](adr/0014-async-shared-world-strategy.md)) → 1.4.0 Descent link**. "In progress" → 1.3.5 next; the *Planned* § carries the 1.3.4–1.3.7 design blurb. |
