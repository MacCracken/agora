...
| 🟣 **Proposed (forward-looking)** | 1 | **ADR 0014** (war-game async shared-world strategy, the MUD on-ramp, planned 1.3.7) — a decision recorded ahead of the build, like ADR 0010 was at 1.2.0-open. Moves to Accepted when its work cuts. (**ADR 0013** moved Proposed→**Accepted** at the 1.3.4 Wager cut.) |
...
| `CHANGELOG.md` | 2026-06-09 | ✅ Fresh | **Source of truth per CLAUDE.md.** [0.1.0] → **[1.3.6]** entered. [1.3.6] = Olympiad (games-owner sim + the `compete()` primitive, ADR 0016; the wager flagship — `wager_payout` fractional book; 12-meet ladder to the crown; solo save + `scores olympiad`) + smoke 17; [1.3.5] = Casino integrations (wager module embedded across PA/Smuggler/QUEST — first user-reachable wager surface; cantina/dice/cards) + pin 6.1.14→6.1.15 + smoke 16; [1.3.4] = Wager (shared casino primitive, ADR 0013 Accepted; built ahead of consumers, DCE-NOPed) + pin 6.1.12→6.1.14 + review fixes; [1.3.3] = Jabberwacky (corpus-learning bot, ADR 0015) + pin 6.1.10→6.1.12 + review fixes; [1.3.2] = QUEST (LORD-homage RPG) + pin 6.1.9→6.1.10 + 3 review fixes; [1.3.1] PARRY; [1.3.0] Chat + Eliza (ADR 0011); [1.2.0] Persistent Universe (ADR 0010); [1.1.x] door games; [1.0.0] release-narrative. |
...
| `VERSION` | 2026-06-09 | ✅ Fresh | `1.3.6`. Bumped via release flow. |
| `cyrius.cyml` | 2026-06-08 | ✅ Fresh | Toolchain pin **`6.1.15`** (realigned 6.1.14 → 6.1.15 at the 1.3.5 cut, 6.1.12 → 6.1.14 at 1.3.4, 6.1.10 → 6.1.12 at 1.3.3, 6.1.9 → 6.1.10 at 1.3.2, 6.1.5 → 6.1.9 at 1.3.0; earlier lifted from the 6.0.52 sigil-SIGILL cap; see state.md Version table). Deps list = 20 stdlib modules (no new deps through 1.3.x — chat/Eliza/PARRY/QUEST/Jabberwacky/Wager + the 1.3.5 casino integrations are all pure; the CSPRNG draw uses the already-listed `syscalls`). |
...
| File | Last touched | Status | Action |
|---|---|---|---|
| `state.md` | 2026-06-09 | ✅ Fresh | **Rotates every release.** **1.3.6 CUT** — Olympiad (197 tests; 841,824 B; pin 6.1.15). Refresh line / Released / Toolchain / Build-artifacts / Tests / In-flight (Olympiad summary + 1.3.7 war-game next) / Recent-shipped rows synced. |
| `roadmap.md` | 2026-06-09 | ✅ Fresh | Release table: **1.3.6 ✅ Olympiad** ([ADR 0016](adr/0016-olympiad-competition-primitive.md)); **1.3.5 ✅ Casino integrations**; **1.3.4 ✅ Wager** ([ADR 0013](adr/0013-wagering-module-rng-fairness.md)); planned **1.3.7 war-game ([ADR 0014](adr/0014-async-shared-world-strategy.md)) → 1.4.0 Descent link**. "In progress" → 1.3.7 next. |
...
16 ADRs (0001-0013 + 0015 + 0016 Accepted; 0014 Proposed for the 1.3.7 work). Re-read pass per minor closeout; ADRs document decisions, not status.
...
| `0014-async-shared-world-strategy.md` | 2026-06-08 | 🟣 Proposed (1.3.7) | **New** — the war-game door as the MUD on-ramp. Async shared-world strategy (common map + alliances + turn-batched combat) extends ADR 0010's flock'd world-transaction; turn resolution is **lazy, on the next caller's entry** (mirrors QUEST's `qu_day_tick`), **no daemon in the first cut** — but a resolution **daemon is kept explicitly OPEN as a deferred future feature** (the likely first long-lived process, toward the MUD's real-time loop). Deliberately rehearses shared-state-mutation-between-callers before 1.4.0. |
