...
> **Last refresh**: 2026-05-23 (0.3.0 released; M5-A/B/C/H landed — agora is a working BBS over the wire with sane UX + security) | **Refresh cadence**: when docs are touched, update the affected row.
...
| `CHANGELOG.md` | 2026-05-23 | ✅ Fresh | **Source of truth per CLAUDE.md.** [0.1.0] scaffold + [0.2.0] M1 close + [0.3.0] M2 close + [Unreleased] ADR 0002 + M5-A post storage + M5-B in-session command interpreter + M5-C sorted listing + M5-H ingress input filter. |
...
| File | Last touched | Status | Action |
|---|---|---|---|
| `state.md` | 2026-05-23 | ✅ Fresh | **Rotates every release.** 0.3.0 (M2 close) shipped; M5 cycle in flight — ADR 0002 + M5-A/B/C/H landed 2026-05-23 (post storage, telnet-side command interpreter, sorted listing, ingress input filter); 32 tests; 116,904 B. Remaining: D (headers), E (boards), F (threads), G (lock). |
